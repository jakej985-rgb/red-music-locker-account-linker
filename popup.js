// Popup logic for Red Music Locker Companion Extension
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  chrome.storage.local.get(["red_music_locker_server", "ytm_sync_server"], (res) => {
    if (res.red_music_locker_server) {
      serverInput.value = res.red_music_locker_server;
    } else if (res.ytm_sync_server) {
  ...
  linkBtn.addEventListener("click", async () => {
    ...
    if (!serverUrl) {
      showStatus("Please enter your Red Music Locker server URL", true);
      return;
    }

    chrome.storage.local.set({ red_music_locker_server: serverUrl });

    linkBtn.disabled = true;
    btnText.textContent = "Connecting...";

    try {
      // 1. Request new auth session from Red Music Locker
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          ...
          showStatus(`✓ Successfully linked as ${userName}! You can now return to Red Music Locker.`);
        }
      );
    } catch (err) {
      linkBtn.disabled = false;
      btnText.textContent = "Link Account Now";
      showStatus(err.message || "Failed to reach Red Music Locker server", true);
    }
  });
